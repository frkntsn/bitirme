...
# Model önbelleği proje içinde (izin sorunlarını önler)
_torch_home = os.getenv("TORCH_HOME", "").strip()
if _torch_home:
    os.environ["TORCH_HOME"] = str((BASE_DIR / _torch_home).resolve())
elif not os.getenv("TORCH_HOME"):
    _default_torch = BASE_DIR / "cache" / "torch"
    _default_torch.mkdir(parents=True, exist_ok=True)
    os.environ["TORCH_HOME"] = str(_default_torch)

_hf_home = os.getenv("HF_HOME", "").strip()
if _hf_home:
    os.environ["HF_HOME"] = str((BASE_DIR / _hf_home).resolve())
elif not os.getenv("HF_HOME"):
    _default_hf = BASE_DIR / "cache" / "huggingface"
    _default_hf.mkdir(parents=True, exist_ok=True)
