...
def segment_region(
    ...
    """SAM ile bölge kes: daire → nokta ipucu; kutu/çizim → kutu ipucu."""
    predictor = _load_sam()
    ...
    if predictor is None:
        if bbox and len(bbox) >= 4:
            ...
            return _fallback_crop(image, x + w / 2, y + h / 2, max(w, h) / 2)
        return _fallback_crop(image, cx, cy, radius)

    predictor.set_image(img_np)
    use_box = shape in ("rect", "brush") and bbox and len(bbox) >= 4
    ...
    crop = _crop_from_mask(image, mask, cx, cy, ref_r)
    return {
        "mask": mask,
        "crop": crop,
