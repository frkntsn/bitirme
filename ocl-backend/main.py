...
async def lifespan(app: FastAPI):
    ...
    # DINOv2'yi başlangıçta yükle (health doğru backend adını göstersin)
    if vlm_model.is_loaded():
        logger.info(f"VLM hazır: {vlm_model.loaded_backend_name()}")
    else:
        logger.warning("VLM yüklenemedi — ilk istekte tekrar denenecek")
    yield
...
def segment(req: SegmentRequest):
    ...
    for ann in req.annotations:
        try:
            seg = segmentation.segment_region(
                image,
                ann.x,
                ann.y,
                ann.radius,
                bbox=ann.bbox,
                shape=getattr(ann, "shape", "circle") or "circle",
            )
        except Exception as e:
